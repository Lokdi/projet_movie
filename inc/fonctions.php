...
// Fonction d'affichage On/Off d'éléments HTML à partir du nom de la page passée en paramètre
function isOnPage($page_ref = 'index.php') {
  $page = basename($_SERVER["PHP_SELF"]);
  if (!empty($page) && $page == $page_ref) {
    return true;
  }
  return false;
}
...
/* FONCTIONS DE GENERATION D'UN SLUG */

// fonction pour générer le slug à partir du titre d'un film
// Param : $str => chaîne de caractère
// Retourne une nouvelle chaîne de caractère sans accent de la forme : khfgf-lifljhf-ehfdkh
/* function slug($str){
   $slug = generate_slug(remove_accent($str));
   return $slug;
 } */

// fonction de généreration d'un slug à partir d'une chaîne de caractères
// Param : $str => chaîne de caractère
// Retourne une chaîne de caractère avec les accents de la forme : khféf-liflâhf-ehfdkh
...
// fonction de remplacement des caratères accentués (UTF-8)
// Param : $str => chaîne de caractère
// Retourne la même chaîne de caractère sans accent
